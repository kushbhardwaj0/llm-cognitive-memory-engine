...
async def main():
    ...
    if not is_healthy:
        ...
        print("   Please make sure Ollama is running (`ollama serve`) and model exists (`ollama pull qwen2.5`).")
        sys.exit(1)
    ...
    extract_prompt = "Extract key relationship: 'Kush is building a Cognitive Memory Engine in Python.'"
    try:
        extracted: ExtractedConcept = await client.generate_json(
            prompt=extract_prompt,
            schema=ExtractedConcept
        )
        print(f"✅ Parsed Pydantic Object successfully:")
